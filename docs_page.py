...
<tr><td><code>webapp.py</code></td><td>The normal browser client. It renders Home, Browse, About, optional Docs, Flipnote, Creator's Room, account and admin pages and exposes thumbnails, inline PPM playback media and counted PPM downloads.</td></tr>
...
<tr><td><code>web/static/site.css</code></td><td>Complete browser stylesheet: colour variables, header/navigation, Flipnote grids, watch/player layout, playback controls, comments, forms, admin tables, documentation layout and responsive breakpoints.</td></tr>
<tr><td><code>web/static/flipnote-player.js</code></td><td>Small browser-side controller around flipnote.js. It loads the PPM, updates play/pause, seek, sound, loop and time controls, and restores the first-frame fallback if playback is unavailable.</td></tr>
...
<tr><td><code>/media/&lt;public id&gt;.ppm</code></td><td>GET</td><td>Returns the original PPM inline for browser playback. It does not increment the download counter.</td></tr>
...
<p>The watch page has browser playback as well as the server-generated first-frame fallback. <code>webapp.py</code> loads the pinned <code>flipnote.js</code> 6.3.1 browser build from jsDelivr, then loads the local <code>web/static/flipnote-player.js</code> controller. The controller creates a <code>flipnote.Player</code> at 320×240, asks it to load <code>/media/&lt;public id&gt;.ppm</code>, and lets flipnote.js decode the original PPM animation and audio in the browser. OSE does not pre-render a video, GIF or separate audio file.</p>
<p>The player starts paused. Once flipnote.js reports that the note is ready, OSE hides the first-frame image and enables the large play button and the control row. The controls provide play/pause, a seek range, elapsed/total time, sound mute/unmute and loop on/off. Seeking uses flipnote.js's <code>startSeek()</code>, <code>seek()</code> and <code>endSeek()</code> calls so playback can resume cleanly after dragging. Clicking the rendered Flipnote toggles playback after the player has been started once.</p>
<p><code>/media/&lt;public id&gt;.ppm</code> is deliberately separate from <code>/flipnote/&lt;public id&gt;.ppm</code>. The media route returns the same on-disk PPM with <code>Content-Disposition: inline</code> and a one-hour public cache header, but it does not call <code>Database.AddDownload()</code>. The download route remains the explicit <strong>Download PPM</strong> action, increments the download counter and returns the file as an attachment. Opening or replaying a watch page therefore does not count as a file download.</p>
<p>The <code>&lt;img&gt;</code> first-frame preview remains in the page before JavaScript runs. If the flipnote.js global is missing, the browser cannot construct its canvas/WebAudio player, the PPM cannot be parsed, or playback reports an error, <code>flipnote-player.js</code> leaves that preview visible, disables the player controls and shows a short playback-unavailable message. The rest of the watch page still works normally. This also gives the page a useful non-JavaScript fallback.</p>
<div class="docs-notice"><strong>Browser dependency/privacy notice.</strong> The playback library is loaded from the jsDelivr CDN rather than copied into OSE. This keeps a large third-party build out of the repository, but it means a browser visiting a watch page makes a request to jsDelivr for <code>flipnote.min.js</code>. The PPM URL itself is same-origin and is not submitted to jsDelivr by OSE. An operator who does not want that third-party browser request should self-host the pinned flipnote.js build and change the script URL in <code>webapp.py</code>.</div>
...
<p>The PPM filename stored in SQLite does not include <code>.ppm</code>. <code>GetFlipnotePPM()</code> reads the whole file. <code>GetFlipnoteTMB()</code> reads only the first <code>0x6A0</code> bytes for menu thumbnails. Browser playback does not create another media file: <code>/media/&lt;public id&gt;.ppm</code> reads and returns this same PPM, while <code>/flipnote/&lt;public id&gt;.ppm</code> reads the same file through the counted download path.</p>
...
<p>The remaining code should be read with that boundary in mind. An OSE Creator's Room is an account-or-IP ownership record; a login is an IP-to-account database row; a ban is an exact IP row; browser playback is a client-side flipnote.js player with a frame-zero fallback; and the server accepts the old proxy-style DSi HTTP path rather than reproducing the removed network/authentication stack.</p>
